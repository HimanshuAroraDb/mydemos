...
# ANSWER
platinumDF = spark.sql("SELECT * FROM mobile_events_delta_gold")
...
# ANSWER  
deviceId = str(spark.sql("SELECT deviceId FROM mobile_events_delta_silver limit 1").first()[0])
...
# ANSWER
spark.sql(f"DELETE FROM mobile_events_delta_silver WHERE deviceId='{deviceId}' ")
noDeviceId = spark.sql(f"SELECT * FROM mobile_events_delta_silver WHERE deviceId='{deviceId}' ").collect()
