...
// MAGIC **A:** The principal difference is that: In a Databricks Delta architecture, output queries can be performed on streaming and historical data at the same time. In a Lambda architecture, streaming and historical data are treated as two separate branches feeding output queries.
// MAGIC 
