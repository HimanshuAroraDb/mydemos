...
val eventsStream = gamingEventDF
  .filter(($"eventParams.client_event_time").isNotNull) 
...
// ANSWER  
val deviceId = spark.sql("SELECT deviceId FROM mobile_events_delta_silver limit 1").first()(0).toString
...
// ANSWER
spark.sql(s"DELETE FROM mobile_events_delta_silver WHERE deviceId='$deviceId' ")
val noDeviceId = spark.sql(s"SELECT * FROM mobile_events_delta_silver WHERE deviceId='$deviceId' ").collect()
...
// ANSWER
spark.sql("VACUUM mobile_events_delta_silver RETAIN 0 HOURS")
