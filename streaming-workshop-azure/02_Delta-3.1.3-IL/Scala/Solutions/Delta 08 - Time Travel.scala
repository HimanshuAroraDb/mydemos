...
spark.sql(s"""ALTER TABLE $powerTable SET TBLPROPERTIES (delta.deletedFileRetentionDuration="interval 10 days") """)
val tblPropDF = spark.sql(s"SHOW TBLPROPERTIES $powerTable")
display(tblPropDF)
...
val countDF = spark.sql(s"SELECT count(*) FROM $powerTable")
display(countDF)

// COMMAND ----------

val historyDF = spark.sql(s"SELECT timestamp FROM (DESCRIBE HISTORY $powerTable) ORDER BY timestamp")
display(historyDF)
...
val historyDF = spark.sql("SELECT timestamp FROM (DESCRIBE HISTORY %s) ORDER BY timestamp".format(powerTable))

// Pick out 2nd write
val oldTimestamp = historyDF.take(2).reverse.head(0).toString
